...
## 20/08/2026 12:00 — Fix — Barcode scan failing: revert scan_mobile document guard — Ram
**Status:** Done — migration 63 written; must be applied to the live project (owner to run via SQL editor). Mobile app (Final-Barcode) unchanged.
**What:** Phone barcode scans stopped working (error surfaced by the app as a generic "Connection" message). Root cause: migration 61 (`scan_mobile_require_document`) added two hard rejections to `scan_mobile` — an INWARD scan was refused unless an `inward_items` row already existed for the unit's batch, an OUTWARD scan unless an `outward_items` row did. Those guards rejected valid scans. Migration 63 reverts `scan_mobile` to the standalone behaviour of migration 59: the reference document (inward_id/outward_id) is derived from the barcode's batch BEST-EFFORT (null is fine — it is nullable on stock_ledger), so a scan still flips the barcode status, posts the ledger, and writes the barcode_scans audit row. Only the two ref-resolution branches differ from mig 61 (each null-check rejection removed); idempotency, the status guards (INWARD requires GENERATED; OUTWARD requires already-inwarded), ledger post, audit row and grants are identical.
**Notes:** Supersedes the guard added in migration 61. The desktop workflow (create the inward/outward entry, then scan each unit) is preserved — when the entry exists, its id is still matched and used as the ledger ref. The Final-Barcode Flutter client (`_scan` in lib/services/api_service.dart) was not changed; it already surfaces server `ok:false` messages and only shows the "Connection" text on a non-PostgrestException failure. Cross-repo: the mobile app lives at C:\Company_Projects\Final-Barcode.
**Files:** supabase/migrations/20260820120000_63_revert_scan_mobile_document_guard.sql

## 20/08/2026 13:30 — Fix — Barcode scan "Could not reach the server": real cause is client product lookup (corrects entry above) — Ram
**Status:** Done — Final-Barcode client edited + analyze clean (no new issues). Migration 63 still stands (separate regression). Owner still to apply migration 63 to the live project.
**What:** The on-screen error the owner reported is "Could not reach the server. Check the backend and network, then scan again." — thrown by barcode_scanner_screen.dart, which fires BEFORE scan_mobile is ever called. The scan flow first calls `getProductByBarcode` → `SupabaseService.lookupProductByCode`, and that is what was throwing. Its catch masked ANY error as a NetworkException. Root cause exposed by migration 54 (silo master data): it dropped the GLOBAL unique constraint on `product_barcodes.code` and made it unique PER OFFICE, and gated reads with `app.can_access_office`. `lookupProductByCode` filtered by `code` only and used `.maybeSingle()`, which throws unless the query returns exactly one row (>1 visible row, or an embedded products/categories/brands/uoms select error). That throw surfaced as the misleading "connection" message. Fixes (Final-Barcode repo): (1) lookupProductByCode now uses `.limit(1)` + first-row instead of `.maybeSingle()`, so a matched-row count can never throw; RLS already scopes the read to the caller's office. (2) getProductByBarcode now rethrows a genuine `PostgrestException` as `ServerErrorException(e.message)` instead of masking it as NetworkException — only real transport failures get the network message. (3) barcode_scanner_screen shows the real `ServerErrorException.message` so any remaining server error is visible instead of a generic line.
**Notes:** This CORRECTS the 12:00 entry, which attributed the failure solely to the migration-61 guard. Migration 61 is a real regression too (it rejects a valid scan once lookup passes, especially OUTWARD), so migration 63 is still the right revert — but it was not the cause of the "Could not reach the server" message the owner saw. Residual possibility: if the barcode's office differs from the scanning user's office, RLS hides it and the app will now correctly show "not found" rather than a false connection error. Cross-repo: client changes live in C:\Company_Projects\Final-Barcode (lib/services/supabase_service.dart, lib/services/api_service.dart, lib/screens/barcode_scanner_screen.dart).
**Files:** (Final-Barcode) lib/services/supabase_service.dart, lib/services/api_service.dart, lib/screens/barcode_scanner_screen.dart

## 20/08/2026 15:00 — UI — Outward Batch Records: group units into clean per-entry colour blocks — Ram
**Status:** Done — desktop typecheck + build clean. UI not exercised here (needs Electron + login + a batch dispatched across multiple outward entries). No DB/backend change.
**What:** With several outward entries against the same product/batch, the Outward Batch Records modal was unreadable: rows were sorted by barcode number while the "Outward Entry" colour was assigned by scan time, so the colours scattered down the table. Owner asked for UI-only clarity and chose the "by scan order" mapping. Changes (BatchBarcodesModal.tsx only): (1) the memo now, besides assigning each dispatched unit to an entry (dispatched ordered by outwarded_at then code, filled into entries in creation order up to each entry's qty — unchanged rule), also returns a grouped `displayRows` (entry blocks first in creation order, then pending/in-stock/void units in code order) and a precomputed `rowClassById`. (2) Each entry block now carries the entry tint PLUS a left accent bar (new `bar` class per palette entry) and a subtle top divider at each block boundary, so groups are unmistakable. (3) The legend and the Outward Entry badge now show a numbered swatch (1, 2, 3…) so the entry sequence is explicit; legend label changed to "Outward entries (in order)". Table now renders `displayRows` with `rowClassById`.
**Notes:** Still a PRESENTATION rule, not ground truth — the DB stores no per-unit → outward-entry link (scan_mobile tags every unit of the batch to one arbitrary outward_items row), so exact per-scan attribution would need a backend change the owner has not requested. Consequence of the "by scan order" grouping: the Barcode Number column is no longer strictly ascending within the outward section (units sit in scan order inside their entry block) — this is the intended trade for clean colour grouping. Adding a new outward entry later shows as the next colour block after the existing ones, filled by units scanned beyond the earlier entries' quantities. Related to the 08/08/2026 "colour units by outward entry" entry.
**Files:** apps/desktop/src/renderer/src/components/barcode/BatchBarcodesModal.tsx

## 20/08/2026 16:12 — Feature — Option B reworked into "Link manufacturer barcode" (quantity-tracked) — Ram
**Status:** Done — desktop typecheck + build clean. UI not exercised live here (needs Electron + login). No DB/backend change. Owner chose "quantity-only" tracking.
**What:** The barcode Generate screen's Option B (Manufacturer Barcode) was broken: it built `quantity` copies of the same code, then on save deduped them to ONE row with `batch_id: null` and hardcoded `symbology: 'CODE128'` — so quantity was discarded, no batch tied the code to any document, and per-unit scanning was impossible (all units share one status row). Root insight: a manufacturer prints the SAME code on every unit, so it is product identity, not a per-unit token — the QUANTITY tracking model (SRD §4 / products.tracking_mode), never the per-unit-serial model that Option A's unique codes use. Reworked Option B in BarcodeEditor.tsx into a one-time "Link manufacturer barcode": inserts exactly one product_barcodes row (batch_id null, real symbology, is_primary only if the product has no code yet). Removed for Option B: the Quantity field, the Batch section, the Format Pattern, the 10× Sequence Review, and the Print button — none apply. Added a Symbology select auto-guessed from the code (new pure `detectSymbology` in lib/barcode.ts: 13 digits→EAN13, 12→UPCA, other numeric→OTHER, mixed→CODE128), user-overridable. Wired the pre-existing but unused `findBarcodeProblem`/`describeBarcodeProblem` validators (written for this exact case) to block save and show the field error; the submit button disables on an invalid/empty code. No barcode is rendered in the Option B preview (a Code 128 picture of an EAN-13 would be a wrong label, and the real symbol is already on the box) — a summary card shows the code, symbology, and primary/alias status instead. Duplicate insert (uq_product_barcode_code, per office) is caught and messaged. Inward/Outward already track quantity at the document level, so those needed no change; stock stays ledger-derived.
**Notes:** office_id continues to come from the column default `app.current_office_id()` (same pattern as the Option A insert — not passed from the client). is_primary is set true only when the product has no existing code, so it can never collide with uq_product_barcode_primary; a "swap primary" flow would need an RPC (two client writes without a transaction would risk leaving the product with no primary) — deliberately deferred. NOT done in this change: the mobile scanner's "how many?" branch for a manufacturer/QUANTITY product — the Final-Barcode Flutter app (C:\Company_Projects\Final-Barcode) still assumes a unique per-unit code and flips ±1 per scan, which is wrong for a shared manufacturer code; that is a separate cross-repo task. Kept the route's existing inline-supabase pattern rather than extracting a hook (would be a drive-by refactor, §14.2).
**Files:** apps/desktop/src/renderer/src/routes/BarcodeEditor.tsx, apps/desktop/src/renderer/src/lib/barcode.ts
