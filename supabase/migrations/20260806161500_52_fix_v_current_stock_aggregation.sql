-- =============================================================================
-- Migration 52: Fix v_current_stock aggregation
--
-- Re-introduces the GROUP BY and SUM() aggregation to v_current_stock, which
-- was accidentally reverted in migration 35. Also includes a CROSS JOIN over
-- offices and products (introduced in migration 33) to ensure products with 0
-- stock still appear in reports.
--
-- This fixes the duplicate rows appearing in Inward and Outward history lists
-- caused by multiple batches for the same product in stock_balances.
-- =============================================================================
