-- =============================================================================
-- Migration 41: Fix save_inward batch conflict target
--
-- Migration 37 inadvertently reverted the ON CONFLICT clause for batch insertion
-- back to the old global constraint `(code)`. Migration 22 changed this constraint
-- to `(product_id, code)`. This restores the correct conflict target.
-- =============================================================================
...
begin
  ...
  insert into public.inwards (
    client_txn_id, office_id, inward_no, supplier_id, supplier_name, supplier_mobile,
    supplier_gst, invoice_no, invoice_date, purchase_order_no, brought_by, notes, invoice_file_path
  ) values (
    p_client_txn_id, v_office_id, v_inward_no, v_supplier_id, trim(p_supplier_name), trim(p_supplier_mobile),
    trim(p_supplier_gst), trim(p_invoice_no), p_invoice_date, trim(p_purchase_order), trim(p_brought_by), trim(p_notes), trim(p_invoice_file_path)
